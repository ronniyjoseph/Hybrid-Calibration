...
def main(labelfontsize = 16, ticksize= 11):
    ...
    contour_levels = numpy.array([5e0, 1e1, 1e2, 1e3, 1e4])

    eta = from_frequency_to_eta(frequency_range)

    eor_power_spectrum = fiducial_eor_power_spectrum(u_range, eta)

    position_raw, position_calibrated = calculate_residual_error(u=u_range, nu=frequency_range, residuals='position', calibration_type='redundant')
    beam_raw, beam_calibrated = calculate_residual_error(u=u_range, nu=frequency_range, residuals='beam',calibration_type='redundant')
    total_raw, total_calibrated = calculate_residual_error(u=u_range, nu=frequency_range, residuals='both',calibration_type='redundant')
    ...
    plot_power_contours(u_range, eta, frequency_range, from_jansky_to_milikelvin(beam_calibrated, frequency_range)/eor_power_spectrum,
                        axes=axes[1], ratio=True, axes_label_font=labelfontsize, tickfontsize=ticksize, xlabel_show=True, norm=ps_norm, ylabel_show=True, contour_levels=contour_levels)
    ...
    plot_power_contours(u_range, eta, frequency_range, from_jansky_to_milikelvin(total_calibrated, frequency_range)/eor_power_spectrum,
                        axes=axes[2], ratio=True, axes_label_font=labelfontsize, tickfontsize=ticksize, xlabel_show=True, norm=ps_norm, ylabel_show=True, contour_levels=contour_levels)
